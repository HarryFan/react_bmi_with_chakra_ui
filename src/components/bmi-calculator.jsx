  ...
  const calculateBMI = () => {
    ...
    if (bmiValue < 18.5) {
      ...
      if (bmiValue >= 24 && bmiValue < 27) {
        setResultText(
          `過重，您需要減掉約 ${weightLossValue} 公斤才能回到正常範圍`
        );
      } else if (bmiValue >= 27 && bmiValue < 30) {
        setResultText(
          `輕度肥胖，您需要減掉約 ${weightLossValue} 公斤才能回到正常範圍`
        );
      } else if (bmiValue >= 30 && bmiValue < 35) {
        setResultText(
          `中度肥胖，您需要減掉約 ${weightLossValue} 公斤才能回到正常範圍`
        );
      } else {
        setResultText(
          `重度肥胖，您需要減掉約 ${weightLossValue} 公斤才能回到正常範圍`
        );
      }
    }
  };
